           ...
           (when @element-visible?
             [:div.application-handling__mass-edit-review-states-popup
              ...
              (when-let [[haku hakukohde] @haku-header]
                [:p
                 @(subscribe [:application/haku-name (:oid haku)])
                 (when-let [name @(subscribe [:application/hakukohde-name (:oid hakukohde)])]
                   (str ", " name))])
...
(defn haku-applications-heading
  ...
  (let [list-opened (r/atom false)
    ...
    (fn [[haku selected-hakukohde hakukohteet]]
       ...
       (if @list-opened
         [:div.application-handling__dropdown-box-wrapper
          [:div.application-handling__dropdown-box-opened
           (haku-row close-list (:oid haku) (nil? selected-hakukohde))
           (doall
            (for [hakukohde hakukohteet]
              ...
              [hakukohde-row
               close-list
               (:oid hakukohde)
               (= (:oid hakukohde) (:oid selected-hakukohde))]))]]
         (closed-row open-list
                     (if selected-hakukohde
                       @(subscribe [:application/hakukohde-name
                                    (:oid selected-hakukohde)])
                       all-hakukohteet-label)))])))
             ...
             (when (not direct-form-application?)
               [:span.application-handling__application-hakukohde-cell
                ...
                (str @(subscribe [:application/hakukohde-name hakukohde-oid])
                     (when-let [tarjoaja-name @(subscribe [:application/hakukohde-tarjoaja-name hakukohde-oid])]
                       (str " - " tarjoaja-name)))])

       ...
       (filter
         (fn [application]
           (let [person-oid (:person-oid application)
                 ...
                 lahtokoulu-oids (suoritus-service/opiskelijan-leikkuripvm-lahtokoulut suoritus-service person-oid haku-oid)]
             (not (empty? (set/intersection authorized-organization-oids lahtokoulu-oids))))))))
...
(defn- filter-jatkuvahaku-applications-by-lahtokoulu
  ...
  (->> applications
       ...
       (filter
         (fn [application]
           (let [person-oid (:person-oid application)
                 hakemuksen-jattohetki (:created-time application)
                 lahtokoulu-oids (suoritus-service/opiskelijan-lahtokoulut suoritus-service person-oid hakemuksen-jattohetki)]
             (not (empty? (set/intersection authorized-organization-oids lahtokoulu-oids))))))))

(defn- filter-applications-by-lahtokoulu
  [tarjonta-service suoritus-service authorized-organization-oids applications]
  (let [jatkuva-haku-applications (filter #(is-for-jatkuva-haku? tarjonta-service %) applications)
        non-jatkuva-haku-applications (remove #(is-for-jatkuva-haku? tarjonta-service %) applications)
        accepted-jatkuva-haku-apps (filter-jatkuvahaku-applications-by-lahtokoulu suoritus-service authorized-organization-oids jatkuva-haku-applications)
        accepted-non-jatkuva-haku-apps (filter-non-jatkuvahaku-applications-by-lahtokoulu suoritus-service authorized-organization-oids non-jatkuva-haku-applications)]
    (map remove-organization-oid (concat accepted-jatkuva-haku-apps accepted-non-jatkuva-haku-apps))))

(defn- filter-authorized-by-lahtokoulu
  [organization-service tarjonta-service suoritus-service session applications authorized-applications]
  (let [opinto-ohjaaja-authorized-organization-oids (organization-oids-for-opinto-ohjaaja organization-service session)]
    (if (and
      ...
      (let [authorized-application-oid? (set (map :oid authorized-applications))
        ...
        (filter-applications-by-lahtokoulu tarjonta-service suoritus-service opinto-ohjaaja-authorized-organization-oids unauthorized-applications))
      [])))

(defn- application-authorized-by-lahtokoulu?
  [organization-service tarjonta-service suoritus-service session application]
  (-> (filter-authorized-by-lahtokoulu organization-service tarjonta-service suoritus-service session [application] [])
    seq
    boolean))
...
(defn filter-authorized-by-session
  ...
  (let [organization-oid-authorized?     (organization-oid-authorized-by-session-pred organization-service session)
        ...
        opo-authorized-applications      (filter-authorized-by-lahtokoulu organization-service tarjonta-service suoritus-service session applications normally-authorized-applications)]
...
(defn- authorize-by-opinto-ohjaaja-fn
  [organization-service tarjonta-service suoritus-service session]
...
(defn- opinto-ohjaaja-access-authorized?
  ...
  (let [[application-authorization-data] (application-store/applications-authorization-data [application-key])]
    (application-authorized-by-lahtokoulu? organization-service tarjonta-service suoritus-service session application-authorization-data)))
...
(defn- applications-opinto-ohjaaja-access-authorized?
  ...
  (let [applications-authorization-data (application-store/applications-authorization-data application-keys)
        authorized-applications (filter-authorized-by-lahtokoulu organization-service tarjonta-service suoritus-service session applications-authorization-data [])]
...
(defn application-edit-authorized?
  ...
  (let [opinto-ohjaaja-fn (authorize-by-opinto-ohjaaja-fn organization-service
    ...
    (or
      ...
      (opinto-ohjaaja-access-authorized? organization-service tarjonta-service suoritus-service session application-key)))) ;necessary as opinto-ohjaaja might not have any regular orgs
...
(defn applications-access-authorized-including-opinto-ohjaaja?
  ...
  (let [opinto-ohjaaja-fn (authorize-by-opinto-ohjaaja-fn organization-service
    ...
    (or (applications-access-authorized? organization-service tarjonta-service session application-keys rights opinto-ohjaaja-fn)
        (applications-opinto-ohjaaja-access-authorized? organization-service tarjonta-service suoritus-service session application-keys)))) ;necessary as opinto-ohjaaja might not have any regular orgs
...
(defn- can-edit-application?
  [organization-service tarjonta-service suoritus-service session application rights-by-hakukohde]
  (or
    (->> rights-by-hakukohde
      (some #(contains? (val %) :edit-applications))
      (true?))
    (application-authorized-by-lahtokoulu? organization-service tarjonta-service suoritus-service session application)))
...
(defn get-latest-application-by-key
  ...
  (let [application         (application-store/get-latest-application-by-key application-key)
    ...
    (when (or (some #(not-empty
      ...
      (-> application
          (assoc :can-edit? (can-edit-application? organization-service tarjonta-service suoritus-service session application rights-by-hakukohde))

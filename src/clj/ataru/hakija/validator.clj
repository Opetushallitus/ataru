...
(defn- passed? [has-applied priorisoivat-hakukohderyhmat answer validators answers-by-key field-descriptor virkailija?]
  (every? (fn [validator]
              (first (async/<!! (validator/validate {:has-applied                  has-applied
                                                     :validator                    validator
                                                     :value                        answer
                                                     :answers-by-key               answers-by-key
                                                     :priorisoivat-hakukohderyhmat priorisoivat-hakukohderyhmat
...
(defn- passes-all?
  [has-applied priorisoivat-hakukohderyhmat validators answers answers-by-key field-descriptor virkailija?]
  (every? true? (map
                  #(passed? has-applied priorisoivat-hakukohderyhmat % validators answers-by-key field-descriptor virkailija?)
                  (or
...
(defn build-results
  [has-applied answers-by-key results priorisoivat-hakukohderyhmat [{:keys [id] :as field} & rest-form-fields] hakukohderyhmat virkailija?]
  (let [id          (keyword id)
    ...
    (into {}
          (if-let [ret (match (merge {:validators []
                              ...
                              (concat results
                                      {id {:passed?
                                           ((validator-keyword->fn validation-keyword) answers-by-key
                                             (build-results has-applied answers-by-key [] priorisoivat-hakukohderyhmat children hakukohderyhmat virkailija?))}})

                              {:fieldClass "wrapperElement"
                               :children   children}
                              (concat results (build-results has-applied answers-by-key [] priorisoivat-hakukohderyhmat children hakukohderyhmat virkailija?))
                                        ...
                                        {id {:passed? (if (or (not (field-belongs-to-hakukohde-or-hakukohderyhma? field))
                                                        ...
                                                        (if (is-question-group-answer? non-empty-answers)
                                                          (and (every? true? (map #(all-answers-allowed? (set %) allowed-values) non-empty-answers))
                                                               (every? true? (map #(passes-all? has-applied priorisoivat-hakukohderyhmat validators (set %) answers-by-key field virkailija?) non-empty-answers)))
                                                          (and (all-answers-allowed? non-empty-answers allowed-values)
                                                               (passes-all? has-applied priorisoivat-hakukohderyhmat validators non-empty-answers answers-by-key field virkailija?)))
                              ...
                              (concat results
                                      {id {:passed? (if (or (not (field-belongs-to-hakukohde-or-hakukohderyhma? field))
                                                            (belongs-to-existing-hakukohde-or-hakukohderyma? field hakukohteet hakukohderyhmat))
                                                      (passes-all? has-applied priorisoivat-hakukohderyhmat validators answers answers-by-key field virkailija?)
                                                      (every? nil? answers))}})
                              :else (when (some? field) (warn "Invalid field clause, not validated:" field)))]
            (build-results has-applied answers-by-key ret priorisoivat-hakukohderyhmat rest-form-fields hakukohderyhmat virkailija?)
            results))))
...
(defn valid-application?
  ...
  (let [answers-by-key     (util/answers-by-key (:answers application))
        extra-answers      (extra-answers-not-in-original-form
                             ...
                             (keys answers-by-key))
        results            (build-results has-applied answers-by-key [] (:priorisoivat-hakukohderyhmat form) (:content form) applied-hakukohderyhmat virkailija?)
        failed-results     (some->>

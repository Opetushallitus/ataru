...
(def kk-application-payment-module-texts
  {:section-title                           {:fi "Hakemusmaksu ja lukuvuosimaksu"
   ...
   :attachment-info                         {:fi "Tallenna liite viimeistään 7 vuorokauden sisällä hakuajan päättymisestä. Määräaika ilmoitetaan liitepyynnön vieressä.\n\nNimeä tiedostot muotoon \"Sukunimi\\_Etunimi\\_dokumentti\", esimerkiksi Meikäläinen\\_Maija\\_tutkintotodistus\n\nSkannaa vaadittavan dokumentin kaikki sivut, joissa on tekstiä, tai ota niistä hyvälaatuiset kuvat. Varmista, että kuvista saa selvää. Kokoa samaan kokonaisuuteen liittyvät sivut yhteen tiedostoon. Esimerkiksi oleskelulupakortin tulisi olla yksi tiedosto, joka voi sisältää useita sivuja. Tarkista, että dokumentit ovat tiedostossa oikein päin.\n\nSuositeltuja tiedostomuotoja ovat PDF, JPG, PNG ja DOCX."
                                             :sv "Spara bilagan senast inom 7 dygn efter att ansökningstiden har utgått. Den angivna tidpunkten syns invid begäran om bilagor. Namnge bilagorna i formen \"Efternamn\\_Förnamn\\_dokument\", t.ex. Svensson\\_Sven\\_uppehållskort. Skanna samtliga textsidor i dokumentet, eller fotografera sidorna med tillräckligt hög kvalitet. Kontrollera att bilderna är tydliga. Samla samtliga sidor som hör till samma helhet i en gemensam fil. T.ex. ska uppehållskortet ingå i en fil, som dock kan innehålla flera sidor. Kontrollera att dokumenten i filen är rättvända. Rekommenderade filformat är PDF, JPG, PNG och DOCX. "
                                             :en "Submit the attachment within 7 (seven) days after the application period has closed. The exact deadline is available next to the attachment request. Name the attachment file(s) in the following way: Lastname\\_First name\\_description/name of document. For example, Smith\\_Mary\\_residence_permit. Scan all the pages of the required document or take good quality pictures. Make sure that the pictures/scans are legible. Combine the pages of the same document into one file. For example, a residence permit should be in one file that can include several pages. Check that the documents are all positioned in the same way upright. Recommended file formats are: PDF, JPG, PNG and DOCX."}

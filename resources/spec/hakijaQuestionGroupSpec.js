    ...
    describe('question group', function() {
      ...
      describe('answering to a dropdown question inside a question group', function() {
        ...
        it('selects correct options', function() {
          expect(
            formFields()
              .eq(14)
              .find('.application__form-select option:selected')
              .text()
      ...
      describe('answering to a multi-choice button inside a question group', function() {
        ...
        it('shows the multi-choice question as answered', function() {
          expect(
            formFields()
              .eq(16)
              .find('.application__form-checkbox:checked:eq(0) + label')
              .text()
          ).to.equal('Lista, monta valittavissa: A')
          expect(
            formFields()
              .eq(16)
              .find('.application__form-checkbox:checked:eq(1) + label')
              .text()
          ).to.equal('Lista, monta valittavissa: B'),
            expect(
              formFields()
                .eq(24)
                .find('.application__form-checkbox:checked + label')
                .text()
            ).to.equal('Lista, monta valittavissa: B')
        })
      })
      ...
      describe('answering to a multi-value text field inside a question group', function() {
        ...
        it('shows the multi-value text field inside a question group as answered', function() {
          expect(
            formFields()
              .eq(18)
              .find('.application__form-text-input:eq(0)')
              .val()
          ).to.equal('Tekstikenttä, monta vastausta: A')
          expect(
            formFields()
              .eq(18)
              .find('.application__form-text-input:eq(1)')
              .val()
          ).to.equal('Tekstikenttä, monta vastausta: B')
          expect(
            formFields()
              .eq(26)
              .find('.application__form-text-input:eq(0)')
              .val()
          ).to.equal('Tekstikenttä, monta vastausta: C')
          expect(
            formFields()
              .eq(26)
              .find('.application__form-text-input:eq(1)')
              .val()
          ).to.equal('Tekstikenttä, monta vastausta: D')
        })
      })
        ...
        it('shows the single-answer adjacent text field inside a question group as answered', function() {
          expect(
            formFields()
              ...
              .find('.application__form-text-input:eq(0)')
              .val()
          ).to.equal('Vierekkäiset tekstikentät, yksi vastaus: vastaus A')
          expect(
            formFields()
              .eq(20)
              .find('.application__form-text-input:eq(1)')
              .val()
          ).to.equal('Vierekkäiset tekstikentät, yksi vastaus: vastaus B')
          expect(
            formFields()
              .eq(28)
              .find('.application__form-text-input:eq(0)')
              .val()
          ).to.equal('Vierekkäiset tekstikentät, yksi vastaus: vastaus C')
          expect(
            formFields()
              .eq(28)
              .find('.application__form-text-input:eq(1)')
              .val()
          ).to.equal('Vierekkäiset tekstikentät, yksi vastaus: vastaus D')
        })
      })
        ...
        it('shows the multi-answer adjacent text field inside a question group as answered', function() {
          expect(
            formFields()
              ...
              .find('.application__form-text-input:eq(0)')
              .val()
          ).to.equal('Vierekkäiset tekstikentät, monta vastausta: vastaus A1')
          expect(
            formFields()
              .eq(21)
              .find('.application__form-text-input:eq(1)')
              .val()
          ).to.equal('Vierekkäiset tekstikentät, monta vastausta: vastaus B1')
          expect(
            formFields()
              .eq(21)
              .find('.application__form-text-input:eq(2)')
              .val()
          ).to.equal('Vierekkäiset tekstikentät, monta vastausta: vastaus A2')
          expect(
            formFields()
              .eq(21)
              .find('.application__form-text-input:eq(3)')
              .val()
          ).to.equal('Vierekkäiset tekstikentät, monta vastausta: vastaus B2')
          expect(
            formFields()
              .eq(29)
              .find('.application__form-text-input:eq(0)')
              .val()
          ).to.equal('Vierekkäiset tekstikentät, monta vastausta: vastaus C1')
          expect(
            formFields()
              .eq(29)
              .find('.application__form-text-input:eq(1)')
              .val()
          ).to.equal('Vierekkäiset tekstikentät, monta vastausta: vastaus D1')
          expect(
            formFields()
              .eq(29)
              .find('.application__form-text-input:eq(2)')
              .val()
          ).to.equal('Vierekkäiset tekstikentät, monta vastausta: vastaus C2')
          expect(
            formFields()
              .eq(29)
              .find('.application__form-text-input:eq(3)')
              .val()
          ).to.equal('Vierekkäiset tekstikentät, monta vastausta: vastaus D2')
      ...
      describe('closing the feedback form', function() {
        ...
        it('closes the feedback form and shows the read-only view', function() {
          ...
          expect(readonlyAnswer(15)).to.equal(
          ...
          expect(readonlyAnswer(19)).to.equal(
            'Tekstikenttä, monta vastausta: A'
          )
          expect(readonlyAnswer(20)).to.equal(
            'Tekstikenttä, monta vastausta: B'
          )
          expect(readonlyAnswer(21)).to.equal('Tekstialue: AAAAA')
          expect(adjacentReadonlyAnswer(0)).to.equal(
            'Vierekkäiset tekstikentät, yksi vastaus: vastaus A'
          )
          expect(adjacentReadonlyAnswer(1)).to.equal(
            'Vierekkäiset tekstikentät, yksi vastaus: vastaus B'
          )
          expect(adjacentReadonlyAnswer(2)).to.equal(
            'Vierekkäiset tekstikentät, monta vastausta: vastaus A1'
          )
          expect(adjacentReadonlyAnswer(3)).to.equal(
            'Vierekkäiset tekstikentät, monta vastausta: vastaus B1'
          )
          expect(adjacentReadonlyAnswer(4)).to.equal(
            'Vierekkäiset tekstikentät, monta vastausta: vastaus A2'
          )
          expect(adjacentReadonlyAnswer(5)).to.equal(
            'Vierekkäiset tekstikentät, monta vastausta: vastaus B2'
          )

          expect(readonlyAnswer(22)).to.equal('Pudotusvalikko: B')
          expect(readonlyAnswer(23)).to.equal(
          ...
          expect(readonlyAnswer(26)).to.equal(
            'Tekstikenttä, monta vastausta: C'
          )
          expect(readonlyAnswer(27)).to.equal(
            'Tekstikenttä, monta vastausta: D'
          )
          expect(readonlyAnswer(28)).to.equal('Tekstialue: BBBBB')
          expect(adjacentReadonlyAnswer(6)).to.equal(
            'Vierekkäiset tekstikentät, yksi vastaus: vastaus C'
          )
          expect(adjacentReadonlyAnswer(7)).to.equal(
            'Vierekkäiset tekstikentät, yksi vastaus: vastaus D'
          )
          expect(adjacentReadonlyAnswer(8)).to.equal(
            'Vierekkäiset tekstikentät, monta vastausta: vastaus C1'
          )
          expect(adjacentReadonlyAnswer(9)).to.equal(
            'Vierekkäiset tekstikentät, monta vastausta: vastaus D1'
          )
          expect(adjacentReadonlyAnswer(10)).to.equal(
            'Vierekkäiset tekstikentät, monta vastausta: vastaus C2'
          )
          expect(adjacentReadonlyAnswer(11)).to.equal(
            'Vierekkäiset tekstikentät, monta vastausta: vastaus D2'
          )
        })
      })

...
(describe
  ...
  (around
    ...
    (let [audit-logger (audit-log/new-dummy-audit-logger)
          form-id (jdbc/with-db-transaction
      ...
      (binding [*application-id* (:id application)
                ...
                *payment-property-application-id* (:id payment-property-application)
                *payment-property-application-key* (:key payment-property-application)]
        (try
          ...
          (finally
            ...
            (delete-file (str *payment-property-application-key* "_" *payment-property-application-id* ".xml"))
            (jdbc/with-db-transaction [connection {:datasource (db/get-datasource :db)}]
                               ...
                               WHERE id IN (?, ?, ?, ?)"
                                                      ...
                                                      *payment-property-application-id*])
                                      (jdbc/execute! connection
                                                     ["DELETE FROM forms
                               WHERE id IN (?, ?, ?)"
                                                      form-id
  ...
  (it "should send submit message to ASHA SFTP server for application to form with tutu-payment property"
      (let [r (tutkintojen-tunnustaminen-submit-job-handler
                {:application-id *payment-property-application-id*}
                {:form-by-id-cache form-by-id-cache-mock
                 :koodisto-cache   koodisto-cache-mock})
            message (xml/parse-str (get-file (str *payment-property-application-key* "_" *payment-property-application-id* ".xml")))]
        (should= {:transition {:id :final}} r)
        (should= :message (:tag message))
        (let [case (create-folder-by-type "ams_case" message)]
          (should= *payment-property-application-key* (property-value "ams_studypathid" case))
          (should= "Hakemus" (property-value "ams_title" case)))
        (let [action (create-folder-by-type "ams_action" message)]
          (should= "Hakemuksen saapuminen" (property-value "ams_title" action))
          (should= "01.01" (property-value "ams_processtaskid" action)))))

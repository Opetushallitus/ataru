...
(describe "email util"
  (describe "make email data"
    (it "returns email data as a map"
      (let [data (util/make-email-data "lahettilas@shakki.fi" ["teppo.testaa@testi.fi"] "Otsikko" {:test-param "foobar"})]
        (should= "lahettilas@shakki.fi" (:from data))
  ...
  (describe "render emails for applicant and guardian"
    ...
    (it "renders email for applicant"
      (let [template-params {:application-url "https://linkki.fi" :content-ending "Viestin loppu"}
            ...
            applicant-email-data (util/make-email-data "lahettilas@shakki.fi" ["teppo.testaa@testi.fi"] "Vahvistusviesti" template-params)
            guardian-email-data (util/make-email-data "lahettilas@shakki.fi" [] "Vahvistusviesti" template-params)
    ...
    (it "renders email for applicant and guardian"
      (let [template-params {:application-url "https://linkki.fi" :content-ending "Viestin loppu"}
            stub-render (stub :render)
            applicant-email-data (util/make-email-data "lahettilas@shakki.fi" ["teppo.testaa@testi.fi" "tiina.testaa@testi.fi"] "Vahvistusviesti" template-params)
            guardian-email-data (util/make-email-data "lahettilas@shakki.fi" ["huoltaja.testaa@testi.fi"] "Vahvistusviesti" template-params)
            emails (util/render-emails-for-applicant-and-guardian
                     applicant-email-data
                     guardian-email-data
                     stub-render)]
